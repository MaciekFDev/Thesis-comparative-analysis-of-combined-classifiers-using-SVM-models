...
X, y = sk.datasets.make_circles(n_samples = 5000)

X_train, X_test, y_train, y_test = model_selection.train_test_split(X, y, test_size=0.2)

AdaBclf = AdaBoostClassifier(base_estimator=SVC(probability=True, kernel='linear'), n_estimators=10)
Baggingclf = BaggingClassifier(base_estimator=SVC(probability=True, kernel='linear'), n_estimators=10,random_state=0)
